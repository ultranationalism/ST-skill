...
写卡时**优先使用 toolkit 抽象**，不需要直接操作 TavernHelper API。只有 toolkit 不覆盖的场景（流式 token 事件监听、世界书操作、自定义 ordered_prompts 编排）才需要查阅 `docs/tavernhelper-api.md` 或 `fullfront-prompt` skill。
...
```js
// generateRaw 模式（默认）— 完全自控上下文，适合数据/逻辑通道
const raw = await callAI('提取关键信息', {
  config, history,
  json_schema: MY_SCHEMA,
});

// generate 模式 — 走 ST 预设，适合叙事通道
const raw = await callAI('描写角色进入房间', {
  config, history,
  mode: 'generate',        // 使用 TavernHelper.generate()
});
```
...
| `opts.overrides` | object? | generate 模式下覆盖 ST 预设槽位（见 `fullfront-prompt` skill） |
...
|---|---|
| 完全自控上下文（`ordered_prompts`） | 走 ST 预设（Prompt Manager 编排） |
| 用户预设/世界书/文风不生效 | 用户预设/世界书/文风生效 |
| 可并行多个调用 | 同时只能一个 |
| 适合：数据处理、摘要、逻辑判定 | 适合：叙事、角色扮演、故事生成 |

generate 模式下，`callAI` 自动将 history 中已有的对话通过 `overrides.chat_history` 注入 ST 预设的聊天历史槽位，最新的 prompt 通过 `user_input` 传入。

### 行为

- `config.data.apiSource === 'parent'`：根据 `mode` 调用 `TavernHelper.generate()` 或 `generateRaw()`
- `config.data.apiSource === 'custom'`：通过 `fetch` 调用 OpenAI 兼容端点（mode 不影响）
...
| 场景 | 需要的 skill |
|---|---|
| 流式 token 事件监听 (`js_stream_token_received_incrementally`) | `docs/tavernhelper-api.md` |
| 世界书读写 (`getLorebookEntries` / `setLorebookEntries`) | `docs/tavernhelper-api.md` |
